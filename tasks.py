...
def clean(ctx, docs=True, bytecode=True, builds=True):
    ...
    with chdir(BASE_FOLDER):
        ...
        if builds:
            ...
            folders.append("src/compas_invocations.egg-info/")
...
def build_ghuser_components(ctx, gh_io_folder=None, ironpython=None):
    ...
    with chdir(BASE_FOLDER):
        with tempfile.TemporaryDirectory("actions.ghcomponentizer") as action_dir:
            ...
            ctx.run(
                "git clone https://github.com/compas-dev/compas-actions.ghpython_components.git {}".format(action_dir)
            )
            ...
            ctx.run(
                '{} {} {} {} --ghio "{}"'.format(ironpython, componentizer_script, source_dir, target_dir, gh_io_folder)
            )
